...
public class GestionPersonnes {
	/*
	 * valide les données pour permetre à un adhérent de se connecter avec un numéro
	 * de téléphone pour afficher les information de son dossier. affiche celles ci
	 * si tout est valide
	 */
	public static void connectionAdh(String strNoTell, Stage stage) {
		...
		if (strNoTell != null && strNoTell != "") {
			...
			Alert alerteChaineVide = new Alert(AlertType.WARNING, "veuillez insrire un numéros de téléphone",
					ButtonType.OK);
			alerteChaineVide.showAndWait();
		}

		if (personneTrouve == null) {
			Alert alerteNonTrouve = new Alert(AlertType.WARNING,
					"Le numéros de téléphone entre ne correspond à aucun dossier", ButtonType.OK);
			alerteNonTrouve.showAndWait();
		} else {
			Alert alerteDossier = new Alert(AlertType.INFORMATION, ((Adherent) personneTrouve).toString(),
					ButtonType.OK);
	...
	/*
	 * valide les données pour permetre à un adhérent de se connecter pour afficher
	 * les information de son dossier. affiche celles ci si tout est valide
	 */
	public static void connectionAdh(String strNom, String strPrenom, Stage stage) {
		...
		if (strNom != null && strNom != "" && strPrenom != null && strPrenom != "") {
			...
			Alert alerteChaineVide = new Alert(AlertType.WARNING, "Veuillez insrire votre nom ET prénom",
					ButtonType.OK);
			alerteChaineVide.showAndWait();
		}

		if (personneTrouve == null) {
			Alert alerteNonTrouve = new Alert(AlertType.WARNING, "Votre nom ne correspond à aucun dossier",
					ButtonType.OK);
			alerteNonTrouve.showAndWait();
		} else {
			Alert alerteDossier = new Alert(AlertType.INFORMATION, ((Adherent) personneTrouve).toString(),
	...
	/*
	 * permet à l'admin ou aux préposés de se connecter afin d'accéder à l'interface
	 * pricipale
	 */
	...
	/*
	 * valide les données néscésaires pour ajouter un adhérent si celle-ci sont
	 * valides crée et ajoute l'adhérent à la liste de personnes. affiche un message
	 * si une donnée n'est pas adéquate
	 * 
	...
	/*
	 * valide les données néscésaires pour ajouter un Préposé si celle-ci sont
	 * valides crée et ajoute l'adhérent à la liste de personnes. affiche un message
	 * si une donnée n'est pas adéquate
	 * 
	 * retourne : true si la personne à été ajoutée
	 ...
	 * 
	 * retourne: null si toute les données sont valides, sinon retourne un message
	 * d'erreur
	 */
	...
	/*
	 * validation de données pour la modification d'un adhérent. modifie celui-ci si
	...
	/*
	 * validation de données pour la modification d'un prépose. modifie celui-ci si
	 * les données sont valides.
	 */
	public static void modifierPrepose(Prepose prep, String strAdresse, String strNoTelephone, String strPwd,
			String strPwdConfirmer, Stage stage) {
		Alert alerteModifierPersonne = ajouterDeBase(prep.getStrNom(), prep.getStrPrenom(), strAdresse, strNoTelephone);
		if (strPwd == null || strPwd.trim().equals("") || strPwdConfirmer == null
				|| strPwdConfirmer.trim().equals("")) {
			alerteModifierPersonne = new Alert(AlertType.WARNING,
					"veuillez insrire le mot de passe ainsi que sa confirmation", ButtonType.OK);
		}
		if (!strPwd.equals(strPwdConfirmer)) {
			alerteModifierPersonne = new Alert(AlertType.WARNING,
					"Le mot de passe ainsi que sa confirmation doivent êtres identiques", ButtonType.OK);
		}
		if (alerteModifierPersonne == null) {
